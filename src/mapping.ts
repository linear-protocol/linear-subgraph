import { near, BigInt, log,json, TypedMap, JSONValue, Value, bigInt, BigDecimal  } from "@graphprotocol/graph-ts";
    ...
    for (let logIndex = 0; logIndex < outcome.logs.length; logIndex++) {
      ...
      if (outcomeLog.includes('EVENT_JSON:')){
        ...
        if (event.toString() == "stake"){
          ...
          price.LinearNum =  minted_shares
          price.NEARNum = stakeAmount
    ...
    for (let logIndex = 0; logIndex < outcome.logs.length; logIndex++) {
      ...
      if (outcomeLog.includes('EVENT_JSON:')){
        ...
        if (event.toString() == "unstake"){
          ...
          log.info('start handle {}',['price'])
          let price = new Price(blockHeight.toString())
          price.NEARNum = unstakeAmount
          price.LinearNum = burnedShares
          price.timeStamp = timeStamp.toString()
          price.save()
        }
      }
    ...
    for (let logIndex = 0; logIndex < outcome.logs.length; logIndex++) {
      ...
      if (outcomeLog.includes('EVENT_JSON:')){
        ...
        if (event.toString() == "liquidity_pool_swap_fee") {
          ...
          let poolFeesObj = dataObj.get('total_fee_shares')
          if (!poolFeesObj) {
            log.info('poolFeesObj {}',['null'])
            return
          }
          log.info('get pool fees {}',[poolFeesObj.toString()])
          let lpapy = new LpApy(blockHeight.toString())
          lpapy.feesPayed = BigInt.fromString(poolFeesObj.toString())
          lpapy.timeStamp = timeStamp.toString()
          lpapy.save()
          log.info('finish to save {}',[blockHeight.toString()])
        }
      }
    }
  }
