...
[LiNEAR](https://linearprotocol.org/), a liquid staking protocol built on NEAR, is the first top-tier project in the NEAR ecosystem that adopts The Graph in production. 

The integration makes it possible for the important metrics on LiNEAR, such as staking APY, liquidity pool APY, and users' staking rewards, to be  queried from The Graph based on [NEAR Event Standard](https://nomicon.io/Standards/EventsFormat), which replaces the previous less flexible and efficient solution based on NEAR Indexer. 

The LiNEAR engineering team would like to share how we're building production-ready subgraph for our applications, and hope the experience we learnt would also be helpful for NEAR DApp developers who'd like to build your first subgraph today. 
...
If you're familiar with the practices of building with The Graph in Ethereum, it's a common practice to handle the events from smart contracts using subgraph. We're following the same practice on NEAR.
...
3. The `event` field is the event name, e.g. `ft_transfer`, `ft_mint`, `ft_burn` for fungible tokens.
...
If you have built your contracts based on [`near-contract-standards`](https://github.com/near/near-sdk-rs/tree/master/near-contract-standards) crate, such as [fungible token](https://examples.near.org/FT) and [non-fungible token](https://examples.near.org/NFT), you already have the built-in events implementation in your contract. So you can use that in subgraph directly. 
...
It's quite common that you need to define you own events in your contract. 

Here we'll implement the events in LiNEAR as an example. LiNEAR is an liquid staking protocol that you could stake $NEAR and receive liquid $LiNEAR token while still earning staking rewards, so all the events for the major activities needs to be created.

We'll define all the events for LiNEAR under [`events.rs`](https://github.com/linear-protocol/LiNEAR/blob/main/contracts/linear/src/events.rs) in the contract project. 
...
(2) We'll define `enum Event` with all the event data schema as enums. 
...
(4) If your contract contains a bunch of different events, we suggest you create unit tests for your events to make sure the generated EVENT_JSON log looks exactly as you think. Running `Event::Stake{...}.emit()` will output the EVENT JSON log. 
...
We'll illustrate how to emit events for stake, unstake and epoch stake in LiNEAR.

(1) `Stake` event is emitted in `internal_stake()` which is called by all stake functions. The account ID, balances, staked $NEAR amount, and minted $LINEAR are recorded in the event. Also, one standard `FtMint` event is emitted since $LINEAR is minted for the user after staking. 
...
More details can be found [here](https://github.com/linear-protocol/LiNEAR/blob/2c78f26084bc8e999cea9643c0f7bf3c6aef06f5/contracts/linear/src/internal.rs#L180-L200).


(3) One thing needs to pay attention to in NEAR is that, because the cross-contract call is asynchronous, to make sure the events are emitted in the expected status (e.g. entire transaction has been executed successfully), the events should be emitted in the appropriate functions or callbacks. 
...
Then we emit the epoch state result in the [callback function](https://github.com/linear-protocol/LiNEAR/blob/2c78f26084bc8e999cea9643c0f7bf3c6aef06f5/contracts/linear/src/epoch_actions.rs#L360-L383).
...
Next, you can process event data through indexing technologies, such as [The Graph](https://thegraph.com), [NEAR Indexer](https://near-indexers.io/docs/projects/near-indexer-framework), [NEAR Lake](https://near-indexers.io/docs/projects/near-lake-framework). We recommend building your data solution with The Graph because it's the most flexible, powerful and cost effective solution for DApps. 
...
The general steps and FAQs about how to develop a subgraph on NEAR can be found in [the tutorial by The Graph team](https://thegraph.com/docs/en/supported-networks/near/). We recommend you go through it quickly if you haven't.
...
- `subgraph.yaml`: the subgraph manifest, defining the data sources of interest, and how they should be processed.
...
We'll talk about all the three aspects in the next step with linear-subgraph as the example: https://github.com/linear-protocol/linear-subgraph

But before that, let's ensure we understand our objectives before we start.

### Set your Objectives

Before we start developing subgraph, we should know what kind of info, stats, or insight we want to get out of the event data. 

Some of the data could be queried via RPC from the smart contracts, but some statistics and analytics are easier to be queried from subgraph. We will need subgraph for such cases. 

For LiNEAR, we care about the staking APY, liquidity pool APY, and staking rewards of users, and would like to show the information in the [LiNEAR web frontend](https://app.linearprotocol.org/). We also care about analytics of users, validators, liquidity, etc., which could help improve the protocol.

In this tutorial, we'll briefly talk about how to calculate the **staking APY of LiNEAR Protocol**, which is based on the growth of $LINEAR price in the past 30 days. In order to reach this goal, we need to get the $LiNEAR price at any timestamp with subgraph. 
...
4. *start block*: usually when your contract is deployed
5. *mapping file*: `./src/mapping/index.ts`
6. *entities*: the entities defined in the schema file
7. *handler*: specify the handler function in your mapping file (`handleReceipt`). we use the `receiptHandlers` since the functions and events are processed at receipt level in NEAR
...
The placeholders `{{network}}`, `{{contract}}`, `{{startBlock}}` are populated by different configurations for mainnet and testnet, which are defined in `./config/mainnet.json` and `./config/testnet.json`.
...
The recommended way is to define the schema based on your data queries and analytics target. In the case of LiNEAR, we have defined:

- **User**: tracks the latest states of each user such as first staking time, transferred amount in total, accumulated minted LiNEAR amount in total, etc. which are not available in contract; You probably also need the *User* entity in your application. 
...
- **TotalSwapFee**: records the total paid swap fees to the liquidity pool at any timestamp when it changes
- **Status**: records some global status such as latest version IDs of prices and total swap fees.


The [built-in scalar types](https://thegraph.com/docs/en/developer/create-subgraph-hosted/#built-in-scalar-types) in The Graph's GraphQL API can be found in the below screenshot. 
...
In general, The Graph works by traversing all or some of the blocks of the blockchain, and processing the data (e.g. events) in the block with the handlers designed by us. 
...
By looking at [the contract code](https://github.com/linear-protocol/LiNEAR/blob/2c78f26084bc8e999cea9643c0f7bf3c6aef06f5/contracts/linear/src/epoch_actions.rs#L456-L462), we know `epoch_update_rewards()` function and its callback `validator_get_balance_callback()` will trigger the `EpochUpdateRewards`, we filter the condition by method name and event name, and then call the corresponding event handler `handleEpochUpdateRewards`. (Note: `method == 'epoch_update_rewards'` is actually not needed and can be removed.)
...
In `./src/mapping/epoch-action.ts`, we have implemented the event handler for `EpochUpdateRewards`, which will update the price based on change of NEAR and LiNEAR total amount. 
...
It's also necessary to handle `FtMint` during update staking rewards because around `1%` commission fee is charged and sent to the treasury of LiNEAR. 
...
We can also take a look how the `updatePrice` works in [`./src/helper/price.ts`](https://github.com/linear-protocol/linear-subgraph/blob/b13779eee7c233932b1ed58090c553b75464bdb6/src/helper/price.ts#L5-L36): a new price record will be created and saved in the database, and also update the global latest price version which keeps track of the latest total $NEAR and $LiNEAR amount. 
...
Now we have built the subgraph, it is time to deploy it to [The Graph's Hosted Service](https://thegraph.com/docs/en/hosted-service/deploy-subgraph-hosted/) for indexing. 

First, you need to create your subgraph in the [Hosted Service dashboard](https://thegraph.com/hosted-service/dashboard) by clicking "Add Subgraph", or just visit [this link](https://thegraph.com/hosted-service/subgraph/create?account=All%20Subgraphs). Fill in the necessary description for the subgraph will be good enough.
...
In the playground, you can edit, save and execute your GraphQL queries. In the above screenshot, we have queried 100 users with the fields we're interested in. 
...
First, get the GraphQL endpoint for our subgraph: `https://api.thegraph.com/subgraphs/name/<username>/<subgraph_name>`

Second, create the URQL client. 
...
Third, query the subgraph with the URQL client. 

Here we'd like to query the $LiNEAR price 30 days ago, so we can calculate the staking APY with it. You can find the query examples in the [`./test` folder](https://github.com/linear-protocol/linear-subgraph/blob/b13779eee7c233932b1ed58090c553b75464bdb6/test/price.js#L4-L23). 
...
Now we can get the latest price from contract, and already have queried the price of 30 days ago, we'll be able to calculate the staking rewards with `(price (now) - price (30 days ago)) / 30 * 365`. We made it!!!

*P.S.* At LiNEAR Protocol, we have built a SDK based on the subgraph queries, which is used in our frontend and analytics. Please feel free to [check out](https://github.com/linear-protocol/linear-sdk) if you want to learn more. 
...
You have learnt about the details of building subgraph on NEAR, from implementing and emitting the events in your smart contract, to building, deploying and querying the subgraphs that process your events. 

Besides this tutorial, there are other excellent guides that will help you learn about using The Graph on NEAR.
...
Now it's your time to start building and hacking. If you have any questions, please feel free to [discuss with us in the `linear-protocol/linear-subgraph` repo](https://github.com/linear-protocol/linear-subgraph/discussions). 
